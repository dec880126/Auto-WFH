...
def login_TaiwanJobs(account: str, password: str, verifyCode: str):
    """
    ...
    click2_and_sleep((2100, 750), 0.1)
    
    click2_and_sleep((1500, 725), 0.1)
    enter_string(password)

    click2_and_sleep((1000, 826), 0.1)
...
def login_process():
    ...
    while True:
        ...
        if not is_recognize:
            ...
            sleep(1)
            continue
        print(f'[*]第 {try_login_times} 次嘗試登入中...')
        try_login_times += 1
